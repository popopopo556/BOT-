...
"""
    ...
    def get_detailed_stats(self):
        ...
        # Фильтруем статистику по дням - убираем действия разработчиков
        filtered_daily_stats = defaultdict(lambda: defaultdict(int))
        ...
        # По дням
        detailed += "📅 *Статистика по дням:*\n"
        for date, stats in sorted(filtered_daily_stats.items(), reverse=True)[:7]:  # Последние 7 дней
            detailed += f"\n{date}:\n"
            for action, count in stats.items():
                display_name = ACTION_NAMES.get(action, action)
                detailed += f"  ├ {display_name}: {count}\n"

        # По пользователям (исключаем разработчиков)
        detailed += "\n👤 *Статистика по пользователям:*\n"
        user_stats = {user: actions for user, actions in self.stats["user_actions"].items()
                      if not any(str(dev_id) in user for dev_id in DEVELOPER_IDS)}

        for user, actions in list(user_stats.items())[:10]:  # Первые 10 пользователей
            detailed += f"\n{user}:\n"
            for action, count in actions.items():
                display_name = ACTION_NAMES.get(action, action)
                detailed += f"  ├ {display_name}: {count}\n"
...
# КНОПКИ СТАТИСТИКИ ДЛЯ РАЗРАБОТЧИКА
...
async def detailed_stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Детальная статистика по кнопке"""
    try:
        user_id = update.message.from_user.id
        ...
        if len(detailed) > 4000:
            parts = [detailed[i:i + 4000] for i in range(0, len(detailed), 4000)]
            for part in parts:
                await update.message.reply_text(part, parse_mode='Markdown')
        else:
            await update.message.reply_text(detailed, parse_mode='Markdown')

        logger.info("📋 Детальная статистика запрошена разработчиком")

    except Exception as e:
        logger.error(f"Ошибка в detailed_stats_command: {e}")
...
async def statistics_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда для детальной статистики (только для разработчика)"""
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE, media_type: str, media_name: str):
    """Универсальный обработчик медиа"""
    try:
        user_info = get_user_info(update)
        user_id = update.message.from_user.id
        stats.add_user_action(user_id, f"{media_type}_post", user_info['username'])

        # Получаем медиа и подпись
        media_map = {
            'photo': (update.message.photo[-1] if update.message.photo else None, 'photo'),
            'video': (update.message.video, 'video'),
            'document': (update.message.document, 'document'),
            'animation': (update.message.animation, 'animation'),
            'voice': (update.message.voice, 'voice'),
            'sticker': (update.message.sticker, 'sticker')
        }

        media_obj, media_field = media_map.get(media_type, (None, None))

        if not media_obj:
            await update.message.reply_text("❌ Ошибка: медиа не найдено")
            return
        ...
        if send_method:
            await send_method(
                chat_id=CHANNEL_ID,
                **{media_field: media_obj.file_id},
                caption=f"🎬 *Предложен новый {media_name}*\n\n{user_details}\n💬 *Описание:* {caption}\n\n⏰ _{get_moscow_time()}_",
                parse_mode='Markdown'
            )

        await update.message.reply_text(
            f"✅ *{media_name} отправлен на модерацию!*\n\n"
            "🔄 Возвращаю в главное меню...",
            parse_mode='Markdown',
            reply_markup=main_menu_keyboard(user_id)
        )
        ...
        logger.error(f"Ошибка в handle_media ({media_type}): {e}")
        await update.message.reply_text(
...
# Обработчики конкретных типов медиа
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await handle_media(update, context, 'photo', 'фото')


async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await handle_media(update, context, 'video', 'видео')


async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await handle_media(update, context, 'document', 'файл')


async def handle_animation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await handle_media(update, context, 'animation', 'GIF-анимация')


async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await handle_media(update, context, 'voice', 'голосовое сообщение')


async def handle_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await handle_media(update, context, 'sticker', 'стикер')
...
def main():
    try:
        ...
        # Команды для разработчика (для совместимости)
        ...
        application.add_handler(MessageHandler(filters.Text([
            ...
            "💝 Поддержать разработчика",
            "📈 Быстрая статистика", "📊 Детальная статистика"
        ]), button_handler))
        ...
        print("👑 Для разработчика доступны кнопки:")
        print("   📈 Быстрая статистика")
        print("   📊 Детальная статистика")
        print("💳 Добавлена поддержка ЮMoney")
        print("⏰ Московское время установлено!")
        print("🛡️  Защита от флуда активирована!")
        print("🎯 Улучшенная стабильность интерфейса!")

        # Запуск бота с ожиданием сообщений
        print("🔄 Бот запускается...")
        print("🤖 Бот активен и ожидает сообщений...")
        ...
        print(f"❌ Бот не смог запуститься: {e}")
    finally:
